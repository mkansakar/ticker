...
dt         <- read.csv("DJI.csv", header = T, sep=',',  quote = "", stringsAsFactors = FALSE)
...
#barplot(tail(vol$Quantity,180), ylab = NULL)
...
bbands.HLC <- BBands( dt[,c("High","Low","Close")] )
bbands.close <- BBands( dt[,"Close"] )
...
lines(alldt$id, alldt$High, col='blue', lty = 1, lwd = 0.15)
lines(alldt$id, alldt$Low, col='brown', lty = 1, lwd = 0.15)
