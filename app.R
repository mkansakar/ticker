...
SCRIPT <- "c:/temp/download_fundamentals.py"
INPUT  <- "c:/temp/symbols.csv"
OUTPUT <- "c:/temp/stock_fundamentals_out.csv"
...
if (file.exists(OUTPUT)) {
  ...
  stop("Fundamentals CSV not found")
}
...
read_stock_data <- function(symbol) {
  file_path <- paste0("c:/temp/dow/", symbol, ".csv")
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      #selectInput("sector", "Select Sector", choices = sectors, selected = sectors[5]),
      #selectInput("symbol", "Select Symbol", choices = NULL), width = 3),
      selectInput("symbol", "Select Symbol", choices = symbols, selected = symbols[1]), width = 3),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$downloadBtn, {
    ...
    source("c:/temp/download_dow_to_csv.R", local = TRUE)
  ...
  shap_data <- eventReactive(input$runShap, {
    ...
    run_shap_engine(
      symbols_file = "c:/temp/symbols.csv",
      data_file    = "c:/temp/market_features.csv"
    )
    read_csv("c:/temp/SHAP_IMPORTANCE_BY_SYMBOL.csv", show_col_types = FALSE)
