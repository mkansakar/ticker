...
dt         <- read.csv("index.csv", header = T, sep=',',  quote = "", stringsAsFactors = FALSE)
dt$date1   <- as.Date(dt$date1, format="%m/%d/%Y")
SID        <- "Nepse Index"
#==================================================================================
#SMA MADC BBAND calculation
rsi1    <- RSI(dt$Index, maType = "SMA")
macd1   <- MACD(dt$Index, nFast = 12, nSlow = 26, nSig = 9, maType = SMA, percent = T)
sma10   <- SMA(dt$Index, n = 12)
sma30   <- SMA(dt$Index, n = 26)
...
f_date      <- "2020-01-01"
l_date      <- "2020-03-05"
dt_p        <- alldt %>% filter(alldt$date1 > f_date & alldt$date1 < l_date)
max_id      <- dt_p[which.max(dt_p$Index),9]
min_id      <- dt_p[which.min(dt_p$Index),9] 
max_dte     <- max(dt_p$Index)
min_dte     <- min(dt_p$Index)

fib_ret100  <- max(dt_p$Index)
fib_ret1    <- min(dt_p$Index) 
fib_ret79   <- round(fib_ret100 - (fib_ret100 - fib_ret1) * 0.786,0)
fib_ret62   <- round(fib_ret100 - (fib_ret100 - fib_ret1) * 0.618,0)
fib_ret50   <- round(fib_ret100 - (fib_ret100 - fib_ret1) * 0.500,0)
fib_ret38   <- round(fib_ret100 - (fib_ret100 - fib_ret1) * 0.382,0)
fib_ret24   <- round(fib_ret100 - (fib_ret100 - fib_ret1) * 0.236,0)
#==================================================================================
...
segments(max_id, max_dte, min_id, min_dte, col="blue", lwd = 2)
abline(h = c(fib_ret100, fib_ret79, fib_ret62, fib_ret50, fib_ret38, fib_ret24, fib_ret1), lwd = 1, col = "blue", lty = 1)
...
#abline(v = c(crss.sgnl), lwd = 0.5, col = "red", lty = 3)
...
mtext("Febonacci", side = 4, col = "gray50", line = .5, cex = 0.75)
...
lines(alldt$id, alldt$signal, col='blue', lty = 1, lwd = 0.45)
abline(v = c(p1), lwd = 0.5, col = "gray50", lty = 3)
#abline(v = c(crss.sgnl), lwd = 0.5, col = "red")
#abline(v = c(crss.sma), lwd = 1, col = "blue")
abline(h = c(0))
