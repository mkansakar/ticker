...
macd1   <- MACD(dt$Index, nFast = 10, nSlow = 30, nSig = 9, maType = SMA, percent = T)
sma10   <- SMA(dt$Index, n = 10)
sma30   <- SMA(dt$Index, n = 30)
bband1  <- BBands(dt$Index, n = 30, sd = 2)
#smi     <- SMI(dt$Index)
dt1     <- data.frame(dt, sma10, sma30, rsi1, macd1, bband1)
#===================================================================
...
boxplot(alldt$Index ~ date1, data = soton.df, xlab = "Month", ylab = "Maximum Temperature", main = "Temperature at Southampton Weather Station (1950-1999)"
)
lines(alldt$id, alldt$up, col = 'blue', lty = 1, lwd = 0.15)
lines(alldt$id, alldt$dn, col = 'blue', lty = 1, lwd = 0.15)
lines(alldt$id, alldt$mavg, col = "brown", lty = 3, lwd = 0.15)
...
#legend("topleft", legend = c("Closing", "SMA 10", "SMA 30"), lty = c(1, 1, 1), col = c("red", "blue", "brown"), bty = "n", lwd = c(2.5, 0.15, 0.15), cex = 1, x.intersp = 0.75, y.intersp = .5)
mtext("Bollinger Bands", side = 4, col = "gray50", line = .5, cex = 0.75)
